...
public class Utilities {
    ...
    private static void hurtAndBreak(ItemStack stack, int i, Random rand, @Nullable ServerPlayer serverPlayer, Runnable runnable) {
        int j;
        ...
        if (i > 0) {
            ...
            for (int l = 0; j > 0 && l < i; ++l) {
                if (!DigDurabilityEnchantment.shouldIgnoreDurabilityDrop(stack, j, rand)) continue;
                ++k;
            }
            if ((i -= k) <= 0) {
                return;
